    ...
    private void TraceBehaviour() {
        if (cameraData.MyCameraType == CameraType.CharacterCamera) {
            if (null != cameraData.MyCameraTarget) {
                cameraData.MyObj.transform.position = cameraData.MyCameraTarget.transform.position;
                cameraData.MyObj.transform.rotation = cameraData.MyCameraTarget.transform.rotation;
            }
        }
    }
}