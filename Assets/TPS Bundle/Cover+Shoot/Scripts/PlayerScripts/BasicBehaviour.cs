...
public class BasicBehaviour : MonoBehaviour
{
	...
	void Awake ()
	{
		...
		aimBehaviour = gameObject.AddComponent<AimBehaviour>();
		coverBehaviour = gameObject.AddComponent<CoverBehaviour>();
		shootBehaviour = gameObject.AddComponent<ShootBehaviour>();
		footsteps = new Footsteps();
		footsteps.OnAwake(config, anim);
	}

	private void Start() {
		moveBehaviour.OnStart();
		aimBehaviour.OnStart();
		coverBehaviour.OnStart();
		shootBehaviour.OnStart(config);
	}

	public void SetCam(Transform playerTran, Transform camTranParent) {
		playerCamera = playerTran;
		playerCameraParent = camTranParent;
	}

	void Update()
	{
		if (playerCameraParent != null && camScript == null) {
			camScript = playerCameraParent.GetComponent<ThirdPersonOrbitCam> ();
		}

		moveBehaviour.OnUpdate();
		aimBehaviour.OnUpdate();
		coverBehaviour.OnUpdate();
		shootBehaviour.OnUpdate();
		footsteps.OnUpdate();
